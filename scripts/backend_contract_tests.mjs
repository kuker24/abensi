...
if (!password) {
  throw new Error('ADMIN_PASSWORD wajib diisi untuk backend contract tests.');
}
...
const contractUserPassword = process.env.CONTRACT_TEST_USER_PASSWORD || `Contract#${stamp}!`;
...
const preview = await request('/identity/users/import/preview', {
  method: 'POST',
  headers: auth,
  body: JSON.stringify({ rows: [{ username: `contract.user.${stamp}`, fullName: 'Contract User', role: 'SISWA', password: contractUserPassword }] })
});
assert.equal(preview.summary.invalid, 0);

const importForm = new FormData();
importForm.append('file', new Blob([`username,fullName,role,password\ncontract.file.${stamp},Contract File,SISWA,${contractUserPassword}\n`], { type: 'text/csv' }), 'users.csv');
...
const createdUser = await request('/identity/users', {
  method: 'POST',
  headers: auth,
  body: JSON.stringify({ username: `contract.user.create.${stamp}`, fullName: 'Contract Create', role: 'SISWA', password: contractUserPassword })
});
