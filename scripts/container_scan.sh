...
trivy fs --scanners vuln,secret,misconfig --severity HIGH,CRITICAL --ignore-unfixed --ignorefile .trivyignore.yaml --exit-code 1 --format json --output artifacts/security/trivy-fs.json .
...
for item in "${images[@]}"; do
  ...
  safe_name=${tag//[:\/]/-}
  trivy image --scanners vuln,secret,misconfig --severity HIGH,CRITICAL --ignore-unfixed --ignorefile .trivyignore.yaml --exit-code 1 --format json --output "artifacts/security/trivy-image-${safe_name}.json" "$tag"
