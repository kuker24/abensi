...
async function main() {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      await tx.academicYear.create({ data: { id: yearId, code: `${prefix}-Y`, name: 'Integration Year', active: true } });
      await tx.semester.create({ data: { id: semesterId, academicYearId: yearId, code: `${prefix}-S1`, name: 'Integration Semester', active: true } });
      await tx.classEnrollment.create({ data: { id: `${prefix}-enrollment-1`, classId, studentId, academicYearId: yearId, semesterId, effectiveFrom: dbDate('2026-06-14'), effectiveTo: dbDate('2026-06-20'), createdById: adminId } });
      await tx.classEnrollment.create({ data: { id: `${prefix}-enrollment-2`, classId, studentId, academicYearId: yearId, semesterId, effectiveFrom: dbDate('2026-06-21'), effectiveTo: dbDate('2026-06-30'), createdById: adminId } });
