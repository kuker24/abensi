...
| 12. Supply-chain, Android, release security | PASS (scripts/config/risk acceptance) | pending phase commit | Pinned existing GitHub Actions to full SHAs, added Dependabot, secret scan, license policy, SBOM generation, Trivy wrapper, Android release security/checksum script, and time-limited ExcelJS→UUID risk acceptance. Evidence: secret scan, license check, SBOM generation, npm high audits, typecheck/lint, worker lint passed locally. |
