...
| 14/17. Repository hygiene and CI restructuring | PASS (configuration) | pending phase commit | Added mandatory CI jobs for PostgreSQL integration/concurrency, upgrade migrations, security/SBOM, web UI/a11y/visual, full-stack E2E, backup/restore drill; removed tracked stale backups; expanded `.gitignore`; excluded Playwright specs from Vitest. Evidence: local typecheck/lint/unit tests passed after changes. |
