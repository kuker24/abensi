...
| 3. SessionRoster integrity | PASS (local unit/type/lint) | pending phase commit | Added migration `0030_session_roster_attendance_fk` with preflight/backfill, composite `StudentAttendance(sessionId, studentId)` FK to `SessionRoster`, `confirmedById/correctedById` FKs, no current-enrollment fallback in class summary/roster, audited repair workflow, period names in runtime snapshots. Evidence: Prisma validate, API typecheck/lint, API 19 suites / 151 tests. |
