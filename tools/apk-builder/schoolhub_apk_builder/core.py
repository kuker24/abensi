...
class BuildProfile:
    ...
    download_url: str = ""
    release_notes: str = "APK Android SIAB2 Reader."
...
class ApkBuilderCore:
    ...
    def check_server(self, timeout: int = 12) -> ServerConnectionResult:
        ...
        if ok:
            messages.append("Web SIAB2 siap dipakai untuk APK Android.")
    ...
    def generate_keystore_command(self, keystore: str | Path, alias: str, password: str) -> list[str]:
        return ["keytool", "-genkeypair", "-v", "-keystore", str(keystore), "-alias", alias, "-keyalg", "RSA", "-keysize", "2048", "-validity", "10000", "-storepass", password, "-keypass", password, "-dname", "CN=SIAB2 APK Builder,O=MAN 1 Rokan Hulu,C=ID"]
