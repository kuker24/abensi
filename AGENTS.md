...
- Android reader provisioning is capped at three server-pinned targets: `READER_IDENTITY_01` permits test-only `GATE_IN`/`GATE_OUT`/`MUSHOLA`/`CHECK_ONLY` validation without attendance writes; `READER_GATE_PRAYER_01` and `READER_GATE_PRAYER_02` use those modes with live attendance writes except `CHECK_ONLY` (Uji Coba Card, no write). Administrators issue a short-lived, one-time activation code through `POST /api/v1/device-readers/:id/android/provision-code`; reader secret and stable target modes remain server-controlled. Gate readers expose separate `GATE_IN` ("Scan Datang") and `GATE_OUT` ("Scan Pulang") menus so operators pick direction explicitly instead of the legacy first/second-scan auto-detect; `GATE_OUT` requires a same-day `GATE_IN` and the `MIN_GATE_STAY_MINUTES` guard first.
