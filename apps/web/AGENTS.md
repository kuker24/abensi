...
- Android reader UI presents only three server-pinned production targets (`READER_IDENTITY_01` as Dev Gate Live, `READER_GATE_PRAYER_01`, `READER_GATE_PRAYER_02`), each with live `GATE_IN`/`GATE_OUT`/`MUSHOLA` plus `CHECK_ONLY` (Uji Coba Card, no write). It requests one-time activation codes from `POST /api/v1/device-readers/:id/android/provision-code`; do not expose reader secrets, signing material, or legacy/free-form reader provisioning through this flow.
