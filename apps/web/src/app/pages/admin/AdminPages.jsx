...
import { AlertTriangle, BookOpen, Building2, Calendar, Check, CheckSquare, Clock, Copy, CreditCard, DoorOpen, Download, Eye, FileText, Flag, HelpCircle, KeyRound, ListChecks, Plus, QrCode, Radar, RefreshCw, Save, ShieldCheck, Siren, Smartphone, Users, Wifi, Zap, Activity, TrendingUp, AlertOctagon, ScanLine } from 'lucide-react';
...
function emergencyExpiryIso(time) {
...
export function EarlyCheckoutEmergencyPage({ notify }) {
  ...
  const scopeCount = emergencyScopeLabels(form).length;

  useEffect(() => {
    if (!active) return undefined;
    const timeout = window.setTimeout(state.refresh, Math.max(0, new Date(active.expiresAt).getTime() - Date.now()) + 250);
  ...
  async function activate(event) {
    ...
    try {
      ...
      notify(error.message || 'Mode Pulang Cepat gagal diaktifkan.', 'bad');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function deactivate() {
    ...
    try {
      ...
      notify('Mode Pulang Cepat dinonaktifkan.');
      state.refresh();
    } catch (error) {
      notify(error.message || 'Mode Pulang Cepat gagal dinonaktifkan.', 'bad');
    } finally {
      setSaving(false);
    }
  }
...
export function SettingsPage({ notify }) {
  ...
  const groupedRules = [
    ...
    ['Aturan Staff/TU', [
      ...
      ['requireStaffGateOut', 'Karyawan/TU/operator wajib scan keluar', 'Mencatat kepulangan staff dan operator pukul 15:30–17:30 WIB.']
    ]],
  ...
  return (
    <div className="content">
      ...
      <div className="master-data-layout">
        <div className="master-data-form-panel">
        <Card title={form.id ? 'Edit Catatan' : 'Catat Kejadian'} sub="Opsional tautkan siswa; kosongkan untuk catatan umum.">
        ...
        <div className="master-data-list-panel">
        <Card title="Catatan Hari Ini" sub="Catatan aktif pada tanggal filter di kiri atas.">
          <div className="master-data-table-region">
          <AsyncTable
            state={notes}
            empty={{ title: 'Belum ada catatan di tanggal ini', sub: 'Isi formulir di kiri untuk mencatat kejadian, atau ubah filter tanggal/kategori.' }}
            columns={[
...
export function PersonnelLeaveReviewPage({ user, notify }) {
  ...
  async function review(row, nextStatus) {
    ...
    try {
      ...
      setDecisionNote('');
      leaves.refresh();
      notify(nextStatus === 'APPROVED' ? 'Disetujui. Klik Cetak PDF TTD di baris ini, print, lalu TTD basah di kertas.' : 'Pengajuan ditolak.');
    } catch (error) {
  ...
  async function signDocument(row) {
    ...
    try {
      ...
      setSignNote('');
      leaves.refresh();
      notify('Arsip: TTD basah di kertas sudah dicatat selesai.');
    } catch (error) {
  ...
  async function downloadLetter(row) {
    ...
    try {
      ...
      notify('PDF siap. Print lalu TTD basah offline di kertas.');
    } catch (error) {
