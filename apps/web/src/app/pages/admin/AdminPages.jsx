...
function UsersPanel({ notify }) {
  ...
  async function downloadUserCard(row) {
    ...
    let data = await apiFetch(`/qr-credentials/export/users/${row.id}/card`);
    if (!data.count) {
      await apiFetch(`/qr-credentials/users/${row.id}/generate`, { method: 'POST', body: JSON.stringify({ label: 'QR SIAB2' }) });
      data = await apiFetch(`/qr-credentials/export/users/${row.id}/card`);
    }
    downloadJsonFile(data, `kartu-digital-${row.username || row.id}-${today()}.json`);
    notify(`Data kartu ${row.fullName} siap diunduh. QR lama tidak diganti jika sudah ada.`);
  }
