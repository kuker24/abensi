...
function StudentImportPanel({ notify }) {
  ...
  async function commitImport() {
    ...
    try {
      ...
      setResult(data);
      if (data.committed) {
        const qr = await apiFetch('/qr-credentials/bulk-generate', { method: 'POST', body: JSON.stringify({ label: 'QR Akademik Berkarakter', onlyMissing: true }) });
        setResult({ ...data, qr });
...
export function DevicesPage({ notify }) {
  ...
  const pageCopy = tab === 'qr'
    ? { title: 'Cetak Kartu Akademik Berkarakter', sub: 'Pilih kelas, sistem melengkapi QR resmi, lalu cetak kartu siap pakai.' }
...
function QrCredentialPanel({ notify }) {
  ...
  const [form, set, reset] = useForm({ userId: '', classId: '', label: 'QR Akademik Berkarakter', expiresAt: '', revokeReason: 'Kartu QR dicabut oleh admin karena kartu hilang atau diganti.' });
