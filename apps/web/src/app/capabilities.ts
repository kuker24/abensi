...
export const ROLE_CAPABILITIES: Record<Role | 'DEVELOPER', readonly Capability[]> = {
  ADMIN_TU: [
    ...
    'gateAttendance.read', 'gateAttendance.record', 'attendanceOverrides.create', 'attendanceOverrides.approve', 'attendanceOverrides.revoke', 'devices.read', 'devices.manage', 'reconciliation.read', 'reconciliation.escalate', 'reconciliation.resolve',
    'settings.read', 'settings.manage', 'reports.read', 'reports.export', 'audit.read', 'profile.self.read', 'profile.self.update'
  ],
  OPERATOR_IT: ['devices.read', 'devices.manage', 'gateAttendance.read', 'gateAttendance.record', 'settings.read', 'audit.read', 'profile.self.read', 'profile.self.update'],
  ...
  GURU_PIKET: ['classAttendance.read', 'session.open', 'session.close', 'gateAttendance.read', 'gateAttendance.record', 'attendanceOverrides.create', 'reconciliation.read', 'reconciliation.escalate', 'reports.read', 'profile.self.read', 'profile.self.update'],
