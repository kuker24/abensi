...
- Production provisioning accepts one short-lived, one-time code issued for one of three server-pinned targets: `READER_IDENTITY_01`, `READER_GATE_PRAYER_01`, and `READER_GATE_PRAYER_02` each record live `GATE_IN`/`GATE_OUT`/`MUSHOLA` while `CHECK_ONLY` (Uji Coba Card) stays read-only. Trust returned target device ID and allowed modes; never offer local override or reassign target identity. Home screen shows separate "Scan Gerbang Datang", "Scan Gerbang Pulang", "Scan Mushola", and "Uji Coba Card" buttons when allowed. The legacy combined `GERBANG` mode is never selectable.
