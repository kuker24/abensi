...
export class ReconciliationService {
  ...
  async reconcileSession(sessionId: string, actorId?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const { start: dayStart, end: dayEnd } = businessDayBounds(session.startsAt);
      const rosterStudentIds = studentReconciliationSkipped ? [] : [...new Set(session.rosters.map((roster) => roster.studentId))];
      const involvedUserIds = [session.teacherId, ...rosterStudentIds];

      const gateLogs = await tx.gateLog.findMany({
        where: {
      ...
      const prayerLogs = rosterStudentIds.length
        ? await tx.prayerAttendanceLog.findMany({
            where: {
        ...
        : [];
      const attendancePolicy = rosterStudentIds.length
        ? await tx.attendancePolicy.findUnique({ where: { id: 1 } })
        : null;
      const overrides = rosterStudentIds.length
        ? await tx.attendanceOverride.findMany({
            where: {
              studentId: { in: rosterStudentIds },
              date: dayStart,
              status: 'APPROVED',
      ...
      const requireAshar =
        Boolean(attendancePolicy?.requireStudentAsharForAfternoon) &&
        sessionEndsAtOrAfter(session.endsAt, attendancePolicy?.asharRequiredClassEndTime, 15 * 60);
      ...
      for (const [userId, logs] of gateLogsByUser.entries()) {
        ...
        for (const log of logs) {
          if (log.direction === 'OUT' && !inside) {
            await this.createFlag(tx, ReconciliationFlagType.OUT_TANPA_IN, session.id, userId, {
              outLogId: log.id,
              outAt: log.tappedAt,
          ...
          if (log.direction === 'IN' && inside) {
            await this.createFlag(tx, ReconciliationFlagType.IN_BERULANG, session.id, userId, {
              inLogId: log.id,
              inAt: log.tappedAt,
          ...
          if (log.direction === 'OUT' && inside && lastInAt) {
            ...
            if (minutesSinceIn < 10) {
              await this.createFlag(tx, ReconciliationFlagType.OUT_TERLALU_CEPAT, session.id, userId, {
                outLogId: log.id,
                minutesSinceIn,
      ...
      for (const studentId of rosterStudentIds) {
        ...
        if (!attendance) {
          if (hasGateIn.has(studentId)) {
            await this.createFlag(tx, ReconciliationFlagType.GATE_IN_TANPA_PRESENSI, session.id, studentId, {
              gateIn: true,
              classStatus: null,
        ...
        if ((attendance.status === StudentAttendanceStatus.HADIR || attendance.status === StudentAttendanceStatus.TELAT) && overrideMap.has(studentId)) {
          await this.createFlag(tx, ReconciliationFlagType.HADIR_VIA_OVERRIDE, session.id, studentId, {
            classStatus: attendance.status,
            overrideId: overrideMap.get(studentId)?.id,
            scope: overrideMap.get(studentId)?.scope
        ...
        if (attendance.status === StudentAttendanceStatus.ALPA && hasGateIn.has(studentId)) {
          await this.createFlag(tx, ReconciliationFlagType.BOLOS_KELAS, session.id, studentId, {
            gateIn: true,
            classStatus: attendance.status
          });
          createdFlags += 1;
        ...
        if (attendance.status === StudentAttendanceStatus.ALPA && !hasGateIn.has(studentId)) {
          await this.createFlag(tx, ReconciliationFlagType.ALPA, session.id, studentId, {
            gateIn: false,
            classStatus: attendance.status
          });
          createdFlags += 1;
        ...
        if (
          ...
          !hasGateIn.has(studentId)
        ) {
          await this.createFlag(tx, ReconciliationFlagType.LUPA_TAP_GERBANG, session.id, studentId, {
            gateIn: false,
            classStatus: attendance.status
          });
          createdFlags += 1;
        ...
        if ((attendance.status === StudentAttendanceStatus.HADIR || attendance.status === StudentAttendanceStatus.TELAT) && requireDhuha && !hasDhuha.has(studentId)) {
          await this.createFlag(tx, ReconciliationFlagType.BELUM_SCAN_DHUHA, session.id, studentId, {
            prayer: 'DHUHA',
            classStatus: attendance.status
          });
          createdFlags += 1;
        ...
        if ((attendance.status === StudentAttendanceStatus.HADIR || attendance.status === StudentAttendanceStatus.TELAT) && requireDzuhur && !hasDzuhur.has(studentId)) {
          await this.createFlag(tx, ReconciliationFlagType.BELUM_SCAN_DZUHUR, session.id, studentId, {
            prayer: 'DZUHUR',
            classStatus: attendance.status
          });
          createdFlags += 1;
        ...
        if ((attendance.status === StudentAttendanceStatus.HADIR || attendance.status === StudentAttendanceStatus.TELAT) && requireAshar && !hasAshar.has(studentId)) {
          await this.createFlag(tx, ReconciliationFlagType.BELUM_SCAN_ASHAR, session.id, studentId, {
            prayer: 'ASHAR',
            classStatus: attendance.status,
      ...
      await writeAudit(tx, {
        ...
        resource: 'session',
        resourceId: session.id,
        after: { createdFlags, ...rosterProvenance }
      });
  ...
  private async createFlag(
    ...
    await tx.reconciliationFlag.upsert({
