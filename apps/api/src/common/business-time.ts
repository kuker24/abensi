...
function assertBusinessDateKey(value: string) {
  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(value);
  if (!match) throw new Error(`Invalid business date key: ${value}`);
  const year = Number(match[1]);
  const month = Number(match[2]);
  const day = Number(match[3]);
  if (month < 1 || month > 12 || day < 1 || day > 31) throw new Error(`Invalid business date key: ${value}`);
...
export function addBusinessDays(value: Date | string, days: number) {
  ...
  while (remaining > 0) {
    const { year, month, day } = assertBusinessDateKey(key);
    const nextNoon = new Date(Date.UTC(year, month - 1, day + step, 12 - JAKARTA_UTC_OFFSET_HOURS, 0, 0, 0));
    key = businessDateKey(nextNoon);
    const weekday = businessWeekday(nextNoon);
