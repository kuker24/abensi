...
- Android activation is capped at three server-pinned targets: `READER_IDENTITY_01` permits test-only `GATE_IN`, `GATE_OUT`, `MUSHOLA`, and `CHECK_ONLY` (Uji Coba Card) validation without attendance writes; `READER_GATE_PRAYER_01` and `READER_GATE_PRAYER_02` record gate/mushola normally while `CHECK_ONLY` stays read-only. `POST /api/v1/device-readers/:id/android/provision-code` issues only a short-lived, one-time code for those targets; completion preserves target device ID and server-assigned modes. `recordQrAndroidGateScan` honors an explicit `GATE_IN`/`GATE_OUT` request instead of guessing direction from scan order; legacy `GERBANG` requests still fall back to first/second-scan auto-detect for older APK builds.
