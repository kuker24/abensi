...
- Android activation is capped at four server-pinned targets: `READER_DEV_TEST_01` and `READER_IDENTITY_01` use `CHECK_ONLY`; `READER_GATE_PRAYER_01` and `READER_GATE_PRAYER_02` use `GERBANG` and `MUSHOLA`. `POST /api/v1/device-readers/:id/android/provision-code` issues only a short-lived, one-time code for those targets; completion preserves target device ID and server-assigned modes.
