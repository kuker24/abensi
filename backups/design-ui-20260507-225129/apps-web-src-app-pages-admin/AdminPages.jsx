...
import { AlertTriangle, BookOpen, Calendar, Check, Clock, Copy, CreditCard, Download, Eye, FileText, Flag, HelpCircle, KeyRound, ListChecks, Plus, QrCode, Radar, RefreshCw, Save, ShieldCheck, Smartphone, Users, Wifi, Zap } from 'lucide-react';
...
import { AsyncTable, Avatar, Btn, Card, DataTable, EmptyState, ErrorState, Field, FriendlyEmptyState, HorizontalBarList, LoadingState, PageHead, Pagination, Pill, ProgressRing, QuickActionCard, RoleTaskPanel, SelectInput, SimpleHelpBox, StackedBar, StatusPill, StepGuide, TextInput, TrendChart, statusLabel } from '../../ui';

function StatCard({ k, v, sub, tone = '' }) {
  return <div className="stat"><div className="stat-label">{k}</div><div className="stat-num">{v}</div><div className={`stat-delta ${tone === 'ok' ? 'up' : tone === 'bad' ? 'down' : ''}`}>{sub}</div></div>;
}
...
export function AdminDashboard() {
  ...
  return <div className="content"><PageHead eyebrow="MULAI HARI INI" title="Ringkasan Admin" sub="Halaman awal untuk melihat apa yang perlu dicek hari ini." actions={<><Btn onClick={() => go('/admin/reports')}><FileText size={14} /> Buka laporan</Btn><Btn variant="primary" onClick={() => go('/admin/anomaly')}><Flag size={14} /> Cek masalah</Btn></>} /><RoleTaskPanel tasks={[{ title: 'Cek sesi kelas', desc: 'Lihat kelas yang berjalan, selesai, atau terlewat.', icon: <Radar size={18} />, onClick: () => go('/admin/sessions') }, { title: 'Cek masalah', desc: 'Tindak data yang belum cocok atau perlu alasan.', icon: <Flag size={18} />, tone: 'warn', onClick: () => go('/admin/anomaly') }, { title: 'Buat akun / data', desc: 'Tambah guru, siswa, kelas, mapel, dan pendaftaran.', icon: <Users size={18} />, onClick: () => go('/admin/master-data') }, { title: 'Aktivasi HP Scanner', desc: 'Buat kode aktivasi untuk HP Android gerbang/mushola.', icon: <Smartphone size={18} />, tone: 'ok', onClick: () => go('/admin/devices') }]} />{dashboard.loading ? <LoadingState /> : dashboard.error ? <ErrorState error={dashboard.error} onRetry={dashboard.refresh} /> : <><div className="grid g-4"><StatCard k="Sesi Hari Ini" v={d.sessionsToday ?? 0} sub={`${closed} selesai · ${open} berjalan`} /><StatCard k="Cakupan Presensi" v={`${coverage}%`} sub="Presensi yang sudah tercatat" tone="ok" /><StatCard k="Masalah Aktif" v={d.openFlags ?? itemsOf(flags.data).length} sub="Perlu tindak lanjut" tone="bad" /><StatCard k="Scan Gerbang" v={d.gateTapCount ?? d.gateLogsToday ?? 0} sub="Catatan masuk/keluar" /></div><div className="grid g-3 chart-summary"><Card title="Cakupan presensi" sub="Semakin penuh lingkaran, semakin lengkap data hari ini."><ProgressRing value={coverage} label="Presensi tercatat" sub={`${coverage}% dari data yang masuk`} /></Card><Card title="Status sesi hari ini" sub="Perbandingan sesi selesai, berjalan, dan terjadwal."><StackedBar segments={[{ label: 'Selesai', value: closed, tone: 'ok' }, { label: 'Berjalan', value: open, tone: 'info' }, { label: 'Terjadwal', value: scheduled, tone: 'warn' }]} total={Math.max(1, Number(d.sessionsToday ?? 0) || closed + open + scheduled)} /></Card><Card title="Kondisi cepat" sub="Masalah dan scan gerbang hari ini."><HorizontalBarList data={[{ label: 'Masalah aktif', value: Number(d.openFlags ?? itemsOf(flags.data).length) || 0 }, { label: 'Scan gerbang', value: Number(d.gateTapCount ?? d.gateLogsToday ?? 0) || 0 }]} labelKeys={['label']} valueKeys={['value']} /></Card></div></>}<div className="grid g-3" style={{ marginTop: 18 }}><Card title="Masalah terbaru" sub="Cocokkan data gerbang dan kelas" actions={<Btn size="sm" onClick={() => go('/admin/anomaly')}>Cek</Btn>}><DashboardMiniList state={flags} type="anomaly" /></Card><Card title="Aktivitas terbaru" sub="Aktivitas gerbang dan sesi" actions={<Btn size="sm" onClick={() => go('/admin/live-monitor')}>Lihat lengkap</Btn>}><DashboardMiniList state={live} type="activity" /></Card></div>{trend.loading ? <LoadingState label="Memuat tren…" /> : trend.error ? <ErrorState error={trend.error} onRetry={trend.refresh} /> : <Card title="Tren 7 hari" sub="Cakupan presensi per hari"><TrendChart data={trend.data} /></Card>}</div>;
}
...
function UsersPanel({ notify }) {
  ...
  function applyUserPreset(role) {
    const prefix = role === 'SISWA' ? 'siswa' : role === 'GURU_MAPEL' ? 'guru' : role === 'GURU_PIKET' ? 'piket' : 'operator';
    reset({ id: '', username: `${prefix}.uji`, fullName: role === 'SISWA' ? 'Siswa Uji Coba' : role === 'GURU_MAPEL' ? 'Guru Uji Coba' : role === 'GURU_PIKET' ? 'Guru Piket Uji' : 'Operator Uji', password: '', role, cardStatus: 'ACTIVE' });
  }
  ...
  async function permanentDelete(row) {
    const confirmUsername = window.prompt(`Hapus permanen tidak bisa dibatalkan. Ketik nama akun untuk konfirmasi: ${row.username}`);
...
export function SchedulePage({ notify }) {
  ...
  async function submit(e) { e.preventDefault(); await apiFetch('/schedules/sessions', { method: 'POST', body: JSON.stringify({ ...form, startsAt: new Date(form.startsAt).toISOString(), endsAt: new Date(form.endsAt).toISOString() }) }); reset({ ...form, startsAt: `${date}T07:15`, endsAt: `${date}T08:45` }); sessions.refresh(); notify('Sesi berhasil dibuat.'); }
  async function submitWeekly(e) { e.preventDefault(); await apiFetch('/schedules/weekly', { method: 'POST', body: JSON.stringify({ ...weeklyForm, dayOfWeek: Number(weeklyForm.dayOfWeek), effectiveFrom: new Date(weeklyForm.effectiveFrom).toISOString(), effectiveTo: weeklyForm.effectiveTo ? new Date(weeklyForm.effectiveTo).toISOString() : undefined, roomId: weeklyForm.roomId || undefined }) }); resetWeekly({ classId: '', subjectId: '', teacherId: '', roomId: '', dayOfWeek: '1', startTime: '07:15', endTime: '08:45', effectiveFrom: today(), effectiveTo: '' }); weekly.refresh(); notify('Jadwal mingguan tersimpan.'); }
  async function generate(row) { const result = await apiFetch(`/schedules/weekly/${row.id}/generate`, { method: 'POST', body: JSON.stringify({ from: new Date(date).toISOString(), to: new Date(date).toISOString() }) }); sessions.refresh(); notify(`${result.generatedCount || 0} sesi dibuat, ${result.skippedCount || 0} dilewati.`); }
...
export function DevicesPage({ notify }) {
  ...
  const options = [['android', 'Aktivasi HP Scanner'], ['qr', 'Kartu QR Siswa/Guru'], ['version', 'Versi Aplikasi HP'], ['cards', 'Kartu RFID'], ['readers', 'Alat Lama'], ['scan', 'Input Manual Cadangan']];
  return <div className="content"><PageHead eyebrow="PERANGKAT ABSENSI" title="Aktivasi HP Scanner" sub="Buat kode aktivasi untuk HP Android. Admin cukup pilih tempat HP dipakai, lalu salin kodenya ke aplikasi HP." /><TabBar value={tab} onChange={setTab} options={options} />{tab === 'android' && <AndroidReaderPanel notify={notify} />}{tab === 'qr' && <QrCredentialPanel notify={notify} />}{tab === 'version' && <MobileVersionPanel notify={notify} />}{tab === 'cards' && <CardsPanel notify={notify} />}{tab === 'readers' && <ReadersPanel notify={notify} />}{tab === 'scan' && <ManualQrScanPanel notify={notify} />}</div>;
}
...
function QrCredentialPanel({ notify }) {
  ...
  async function generate() {
    ...
    setResult(data); credentials.refresh(); notify('Kartu QR dibuat. Cetak/salin sekarang.');
  }
  async function rotate() {
    ...
    setResult(data); credentials.refresh(); notify('Kartu QR diganti.');
  }
  async function revoke(row) {
    ...
    credentials.refresh(); notify('Kartu QR dicabut.');
  }
  async function bulk() {
    if (!form.classId && !await riskConfirm('Buat QR untuk semua pengguna aktif?')) return;
    ...
    setResult(data); notify(data.message || 'Buat banyak QR selesai.');
  }
  async function exportCards() {
    const data = await apiFetch(form.classId ? `/qr-credentials/export/class/${form.classId}/cards` : '/qr-credentials/export/cards');
    setResult(data); notify('Data kartu QR siap dicetak dari pratinjau.');
  }
  return <div className="grid management-grid"><Card title="Buat Kartu QR" sub="QR tidak menampilkan data pribadi. Cetak saat dibutuhkan dan cabut jika hilang."><div className="form-grid"><Field label="Pengguna"><SelectInput value={form.userId} onChange={(e) => set('userId', e.target.value)}><option value="">Pilih siswa/guru</option>{itemsOf(users.data).map((u) => <option key={u.id} value={u.id}>{u.fullName} · {statusLabel(u.role)}</option>)}</SelectInput></Field><Field label="Kelas (opsional)"><SelectInput value={form.classId} onChange={(e) => set('classId', e.target.value)}><option value="">Semua / tidak dipilih</option>{itemsOf(classes.data).map((c) => <option key={c.id} value={c.id}>{c.code} · {c.name}</option>)}</SelectInput></Field><Field label="Label"><TextInput value={form.label} onChange={(e) => set('label', e.target.value)} /></Field><Field label="Kedaluwarsa opsional"><TextInput type="datetime-local" value={form.expiresAt} onChange={(e) => set('expiresAt', e.target.value)} /></Field><Btn variant="primary" type="button" onClick={generate}><Plus size={14} /> Buat untuk Pengguna</Btn><Btn type="button" onClick={rotate}><RefreshCw size={14} /> Ganti QR</Btn><Btn type="button" onClick={bulk}>Buat Banyak QR</Btn><Btn type="button" variant="ghost" onClick={exportCards}><Download size={14} /> Cetak Kartu QR</Btn><Btn type="button" variant="ghost" onClick={() => { reset({ userId: '', classId: '', label: 'QR Absensi SchoolHub', expiresAt: '', revokeReason: 'Kartu QR dicabut oleh admin karena kartu hilang atau diganti.' }); setResult(null); }}>Reset</Btn></div></Card><Card title="Daftar QR Pengguna"><AsyncTable state={credentials} columns={[{ header: 'Label', render: (r) => r.label || 'QR Absensi' }, { header: 'Status', render: (r) => <StatusPill status={r.status} /> }, { header: 'Kode Pendek', render: (r) => r.shortCode || '—' }, { header: 'Terbit', render: (r) => formatDateTime(r.issuedAt) }, { header: 'Terakhir Dipakai', render: (r) => formatDateTime(r.lastUsedAt) }]} onRow={(r) => <div className="row"><Btn size="sm" variant="danger" onClick={() => revoke(r)}>Cabut</Btn></div>} /></Card><Card title="Hasil / Kartu QR" sub="QR hanya ditampilkan setelah dibuat atau dicetak oleh admin."><pre className="codeblock">{result ? JSON.stringify(result, null, 2) : 'Belum ada hasil.'}</pre></Card></div>;
}
...
const ANDROID_READER_PRESETS = [
  { key: 'gate-in', icon: '🏫', title: 'Gerbang Masuk', desc: 'HP dipakai saat siswa/guru masuk sekolah.', name: 'HP Gerbang Masuk', locationName: 'Gerbang Masuk', allowedModes: ['GATE_IN'] },
  { key: 'gate-out', icon: '🚪', title: 'Gerbang Keluar', desc: 'HP dipakai saat siswa/guru keluar/pulang.', name: 'HP Gerbang Keluar', locationName: 'Gerbang Keluar', allowedModes: ['GATE_OUT'] },
  { key: 'gate-both', icon: '🔁', title: 'Gerbang Masuk & Keluar', desc: 'Satu HP bisa dipakai untuk masuk dan keluar.', name: 'HP Gerbang Utama', locationName: 'Gerbang Utama', allowedModes: ['GATE_IN', 'GATE_OUT'] },
  { key: 'mushola', icon: '🕌', title: 'Mushola', desc: 'HP khusus scan kehadiran mushola.', name: 'HP Mushola', locationName: 'Mushola', allowedModes: ['MUSHOLA'] },
  { key: 'check-only', icon: '✅', title: 'Coba Dulu / Cek Saja', desc: 'Untuk latihan. QR dicek, tapi tidak mencatat hadir.', name: 'HP Uji Coba Scanner', locationName: 'Uji Coba', allowedModes: ['CHECK_ONLY'] }
];
...
export function LiveMonitorPage() {
  ...
  const state = useRemote(() => apiFetch('/reports/live-monitor?page=1&limit=80'), [auto]);
  useEffect(() => { if (!auto) return; const timer = setInterval(state.refresh, 5000); return () => clearInterval(timer); }, [auto]);
