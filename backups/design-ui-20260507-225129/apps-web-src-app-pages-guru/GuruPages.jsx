...
import { ArrowRight, Check, CheckSquare, Clock, MapPin, Save, Users, Wifi, X } from 'lucide-react';
import { apiFetch, formatDateTime, go, itemsOf, monthNow, qs, today } from '../../api';
import { riskConfirm } from '../../confirm';
import { useRemote } from '../../hooks';
import { Avatar, Btn, Card, DataTable, EmptyState, ErrorState, Field, HorizontalBarList, LoadingState, PageHead, RoleTaskPanel, SelectInput, SimpleHelpBox, StackedBar, StatusDonut, StatusPill, StepGuide, TextInput, statusLabel } from '../../ui';
...
export function TeacherDashboard() {
  ...
  const next = itemsOf(sessions.data).find((s) => s.status === 'OPEN') || itemsOf(sessions.data).find((s) => s.status === 'SCHEDULED') || itemsOf(sessions.data)[0];
  return <div className="content"><PageHead eyebrow="GURU MAPEL" title="Mulai Mengajar" sub="Ikuti urutan sederhana: absen masuk, tandai siswa, simpan, lalu absen keluar." actions={<Btn variant="primary" onClick={() => go('/guru/presensi')}><CheckSquare size={14} /> Isi Presensi</Btn>} /><RoleTaskPanel tasks={[{ title: 'Isi presensi kelas', desc: 'Buka sesi, tandai siswa, lalu simpan.', icon: <CheckSquare size={18} />, tone: 'ok', onClick: () => go('/guru/presensi') }, { title: 'Perbaiki presensi', desc: 'Jika ada kesalahan, koreksi dengan alasan.', icon: <Save size={18} />, onClick: () => go('/guru/koreksi') }, { title: 'Ajukan izin', desc: 'Kirim izin/sakit/dinas luar ke Admin/TU.', icon: <Clock size={18} />, onClick: () => go('/guru/izin') }]} />{sessions.loading ? <LoadingState /> : sessions.error ? <ErrorState error={sessions.error} /> : <>{next && <div className="hero-session"><div className="hero-grid" /><div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}><div><div className="eyebrow" style={{ color: 'var(--accent)' }}><span className="dot" /> SESI UTAMA · {statusLabel(next.status)}</div><div style={{ fontSize: 34, fontWeight: 700 }}>{next.subject?.name} · {next.schoolClass?.code}</div><div className="row muted" style={{ marginTop: 8, gap: 14 }}><span><Clock size={14} /> {formatDateTime(next.startsAt)}</span><span><Users size={14} /> {next.teacher?.fullName}</span></div></div><StatusPill status={next.status} /></div><div className="row" style={{ marginTop: 28, gap: 10 }}><Btn variant="primary" size="lg" onClick={() => go('/guru/presensi')}><ArrowRight size={16} /> Masuk ke sesi</Btn><span className="chip"><MapPin size={12} /> Lokasi sekolah siap</span><span className="chip"><Wifi size={12} /> Terhubung ke server</span></div></div>}{!next && !sessions.loading && <div className="empty" style={{ marginBottom: 18 }}><b>Tidak ada sesi hari ini</b><span>Semua sesi sudah selesai atau belum dijadwalkan.</span></div>}<div className="grid g-3 chart-summary"><Card title="Status sesi" sub="Ringkasan sesi hari ini."><StackedBar segments={sessionSegments(sessions.data)} /></Card><Card title="Kehadiran saya (14 hari)" sub="Ringkasan status kehadiran guru."><StatusDonut counts={countByStatus(mine.data)} title="Kehadiran saya" /></Card></div><div className="grid g-3" style={{ marginTop: 18 }}><Card title="Jadwal sesi hari ini">{itemsOf(sessions.data).length ? <DataTable rows={itemsOf(sessions.data)} columns={[{ header: 'Waktu', render: (r) => formatDateTime(r.startsAt) }, { header: 'Kelas', render: (r) => r.schoolClass?.code || '—' }, { header: 'Mapel', render: (r) => r.subject?.name || '—' }, { header: 'Status', render: (r) => <StatusPill status={r.status} /> }]} /> : <EmptyState title="Tidak ada sesi" sub="Belum ada sesi terjadwal hari ini." />}</Card><Card title="Riwayat kehadiran saya" sub="14 hari terakhir" actions={<Btn size="sm" onClick={() => go('/guru/kehadiran-saya')}>Lihat semua</Btn>}><AttendanceTableState state={mine} /></Card></div></>}</div>;
}
...
export function ClassInputPage({ notify }) {
  ...
  async function closeSession() {
    if (isEarlyCheckout && earlyReason.trim().length < 10) { notify('Isi alasan keluar lebih awal minimal 10 karakter.'); return; }
  ...
  return <div className="content"><PageHead eyebrow="ISI PRESENSI KELAS" title={current ? `${current.subject?.name} · ${current.schoolClass?.code}` : 'Pilih sesi'} sub="Ikuti langkah dari kiri ke kanan agar tidak terlewat." actions={<div className="session-picker-control"><Field label="Pilih sesi"><SelectInput wrapperClassName="session-picker-select" value={sessionId} onChange={(e) => setSessionId(e.target.value)}><option value="">Pilih sesi yang tersedia</option>{itemsOf(sessions.data).map((s) => <option key={s.id} value={s.id}>{s.schoolClass?.code} · {s.subject?.name} · {statusLabel(s.status)}</option>)}</SelectInput></Field></div>} />
    ...
    <div className="grid g-4"><StatCard k="Jam Mulai" v={startsAt ? startsAt.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '—'} sub={teacherPresence?.checkInAt ? `Masuk ${formatDateTime(teacherPresence.checkInAt)}` : 'Guru belum absen masuk'} /><StatCard k="Jam Selesai" v={endsAt ? endsAt.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '—'} sub={teacherPresence?.checkOutAt ? `Keluar ${formatDateTime(teacherPresence.checkOutAt)}` : isEarlyCheckout ? 'Belum waktunya keluar' : 'Sudah boleh absen keluar'} tone={isEarlyCheckout ? 'warn' : 'ok'} /><StatCard k="Status Guru" v={teacherPresence?.status ? statusLabel(teacherPresence.status) : statusLabel(current?.status)} sub="Masuk/keluar kelas" /><StatCard k="Siswa Tercatat" v={roster.length} sub={`${counts.HADIR || 0} hadir · ${counts.ALPA || 0} alpa`} /></div>
    <Card title="Aksi guru" sub="Aksi presensi siswa dipisah dari absen keluar guru agar data awal pembelajaran tidak berubah tanpa sengaja."><div className="row" style={{ gap: 8, flexWrap: 'wrap' }}><Btn variant="primary" onClick={openSession} disabled={!sessionId || isOpen}><Check size={14} /> Absen Masuk / Mulai Kelas</Btn><Btn onClick={() => setRoster((prev) => prev.map((r) => r.eligibility?.locked ? r : ({ ...r, status: 'HADIR' })))} disabled={!isOpen}><Users size={14} /> Tandai semua Hadir</Btn><Btn variant="danger" onClick={() => setRoster((prev) => prev.map((r) => ({ ...r, status: 'ALPA' })))} disabled={!isOpen}><X size={14} /> Kembalikan ke Alpa</Btn><Btn onClick={saveBatch} disabled={!roster.length || !isOpen}><Save size={14} /> Simpan Presensi Awal</Btn><Btn variant="primary" onClick={closeSession} disabled={!roster.length || !isOpen}>Absen Keluar / Akhiri Kelas <ArrowRight size={14} /></Btn></div>{isEarlyCheckout && isOpen && <Field label="Alasan keluar sebelum jam selesai" hint={`${earlyReason.trim().length}/10+`}><TextInput value={earlyReason} onChange={(e) => setEarlyReason(e.target.value)} placeholder="Wajib diisi jika kelas diakhiri sebelum jam selesai" /></Field>}</Card>
    <div className="grid g-2 chart-summary"><Card title="Presensi siswa awal pembelajaran" sub="Cukup isi di awal kelas. Tandai semua Hadir, lalu ubah siswa yang Telat/Izin/Sakit/Alpa."><StatusDonut counts={counts} title="Status siswa" /></Card><Card title="Ringkasan sebelum absen keluar" sub="Periksa ulang sebelum mengakhiri kelas."><StackedBar segments={STATUS.map((st) => ({ label: statusLabel(st), value: counts[st] || 0, tone: st === 'HADIR' ? 'ok' : st === 'ALPA' ? 'bad' : st === 'TELAT' ? 'warn' : 'info' }))} /></Card></div><div className="dock dock-sticky" aria-label="Ringkasan status presensi"><div className="dock-stats">{STATUS.map((st) => <span className="s" key={st}><span className="k">{statusLabel(st)}</span><span className="v">{counts[st] || 0}</span></span>)}</div>{isOpen && <Btn size="sm" variant="primary" onClick={saveBatch} disabled={!roster.length}><Save size={13} /> Simpan</Btn>}</div>{rosterState.loading ? <LoadingState /> : rosterState.error ? <ErrorState error={rosterState.error} /> : <div className="roster">{roster.map((s, i) => <div key={s.studentId} className="roster-row"><div className="roster-idx">{String(i + 1).padStart(2, '0')}</div><Avatar name={s.fullName} /><div><div className="roster-name">{s.fullName}</div><div className="roster-meta">{s.username} · kartu {statusLabel(s.cardStatus)} · {s.eligibility?.locked ? `Terkunci: ${s.eligibility.reasons?.join(', ')}` : 'Syarat scan lengkap/diizinkan'}</div></div><div className="statuspick">{STATUS.map((st) => <button key={st} className={`${s.status === st ? 'on ' : ''}${st.toLowerCase()}`} disabled={!isOpen || (s.eligibility?.locked && ['HADIR', 'TELAT'].includes(st))} title={s.eligibility?.locked && ['HADIR', 'TELAT'].includes(st) ? s.eligibility.reasons?.join(', ') : ''} onClick={() => setStatus(s.studentId, st)}>{statusLabel(st)}</button>)}</div></div>)}</div>}</div>;
}
