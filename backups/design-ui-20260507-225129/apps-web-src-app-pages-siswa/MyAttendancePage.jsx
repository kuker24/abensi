...
import { Card, DataTable, EmptyState, ErrorState, Field, LoadingState, PageHead, RoleTaskPanel, SelectInput, SimpleHelpBox, StatusDonut, StatusPill } from '../../ui';
...
export function MyAttendancePage({ title = 'Kehadiran Saya', student = false }) {
  ...
  return (
    <div className="content">
      ...
      {student && <RoleTaskPanel tasks={[{ title: 'Lihat data hari ini', desc: 'Cek apakah presensi sudah muncul.', onClick: () => go('/siswa/dashboard') }, { title: 'Baca notifikasi', desc: 'Lihat pesan atau tugas dari sekolah.', onClick: () => go('/siswa/notifikasi') }, { title: 'Minta bantuan', desc: 'Jika data salah, hubungi wali kelas atau guru piket.', onClick: () => go('/siswa/panduan'), tone: 'warn' }]} />}
      ...
      <div className="grid g-3">
        ...
        <Card title="Hari ini" sub="Data bisa berubah sampai guru menutup sesi.">
          ...
          {student && (
            <div className="inline-error" style={{ marginTop: 12, background: 'var(--info-soft)', borderColor: 'var(--info)', color: 'var(--info)' }}>
