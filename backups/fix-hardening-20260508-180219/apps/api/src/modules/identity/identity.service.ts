...
export class IdentityService {
  ...
  async previewUsersImport(rows: ImportUserRowDto[]) {
    const normalized = rows.map((row, index) => ({
      ...
      role: row.role,
      password: row.password || 'SchoolHub#2026',
      errors: [] as string[]
    }));
    ...
    for (const row of normalized) {
      ...
      if (row.password.length < 8) row.errors.push('password minimal 8 karakter');
    }
  ...
  async commitUsersImport(rows: ImportUserRowDto[], actor: { sub: string; role: string }) {
    ...
    for (const row of rows) {
      const passwordHash = await bcrypt.hash(row.password || 'SchoolHub#2026', 10);
