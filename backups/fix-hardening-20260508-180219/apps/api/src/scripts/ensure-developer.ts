...
async function main() {
  ...
  if (!configuredPassword && !fallbackPassword && process.env.NODE_ENV === 'production') {
    throw new Error('DEVELOPER_PASSWORD atau ADMIN_PASSWORD wajib diisi untuk production.');
  }
  ...
  const passwordHash = await bcrypt.hash(configuredPassword || fallbackPassword || 'Developer#2026', 10);
