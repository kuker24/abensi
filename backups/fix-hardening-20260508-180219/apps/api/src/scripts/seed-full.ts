...
function envOrDevFallback(name: string, fallback: string) {
  const value = process.env[name];
  if (value) return value;
  if (process.env.NODE_ENV === 'production') {
    throw new Error(`${name} wajib diisi untuk seed production.`);
  }
  return fallback;
}
...
async function main() {
  const defaultPassword = envOrDevFallback('DEFAULT_USER_PASSWORD', 'SchoolHub#2026');
  const adminUsername = process.env.ADMIN_USERNAME ?? 'admin.tu';
  const adminPassword = envOrDevFallback('ADMIN_PASSWORD', 'Admin#12345');
