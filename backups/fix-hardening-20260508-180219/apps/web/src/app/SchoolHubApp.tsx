...
const ROLE_PRESETS: Record<LoginRole, { id: string; password: string; idLabel: string; accountLabel: string }> = {
  guru: { id: 'guru', password: 'guru', idLabel: 'Guru beta: username guru, password guru', accountLabel: 'Guru beta' },
  admin: { id: 'admin', password: 'admin', idLabel: 'Admin/TU beta: username admin, password admin', accountLabel: 'Admin/TU beta' },
  siswa: { id: 'siswa', password: 'siswa', idLabel: 'Siswa beta: username siswa, password siswa', accountLabel: 'Siswa beta' }
};
...
function LoginScreen({ onLogin, mode, onToggleTheme }: { onLogin: (selectedRole: LoginRole, username: string, password: string) => Promise<void>; mode: ThemeMode; onToggleTheme: (theme: ThemeMode) => void }) {
  ...
  const [pw, setPw] = useState(ROLE_PRESETS.guru.password);
  const [err, setErr] = useState('');
  const [loading, setLoading] = useState(false);
  const setRole = (nextRole: LoginRole) => {
    setRoleState(nextRole);
    setId(ROLE_PRESETS[nextRole].id);
    setPw(ROLE_PRESETS[nextRole].password);
    setErr('');
  };
